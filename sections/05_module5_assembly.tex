...
% =============================================================
\newpage
\section{Module 5: Initialization, Equilibration, and System Assembly}\label{sec:mod5}

\subsection{What Are We Doing?}

Having built the tools to compute forces (Module 1), enforce periodic geometry (Module 2), propagate motion (Module 3), and do so efficiently (Module 4), we are now ready to assemble them into a working simulation. But before the engine can run, we must answer two questions that everyone faces when setting up an experiment: \emph{how do we prepare the system in a physically valid state?} and \emph{how long do we wait before we trust any measurement?}

This module solves both problems. It assigns every atom a velocity drawn from the correct statistical distribution for the target temperature, eliminates any artificial centre-of-mass drift, and then runs an equilibration phase, a controlled warm-up period, before the simulation settles into the stable state from which we collect data.

\subsection{Why Are We Doing It?}

\subsubsection{Why Maxwell-Boltzmann Velocities?}

Imagine that you are running at a temperature $T^*$ and you need to assign each atom a starting speed. The naive approach gives every atom exactly the same speed, creates an artificial, crystalline structure in velocity space that does not resemble any real material. In reality, atoms in thermal equilibrium do not move uniformly. Some are slow, some are fast, and their speeds span a continuous range set by the \textbf{Maxwell-Boltzmann distribution}, a direct consequence of classical statistical mechanics.

This distribution tells us that each individual velocity component ($v_x$, $v_y$, $v_z$) of any given atom is a Gaussian random variable with zero mean and a spread determined entirely by temperature. In reduced units, the standard deviation is simply $\sigma = \sqrt{T^*}$: hotter systems have faster, more spread-out velocity distributions. By sampling each component from this Gaussian, we start the simulation as close as possible to a real thermal equilibrium, implying no artificial symmetry and no unphysical ordering.

\subsubsection{Why Zero the Net Momentum?}
\label{sec_Zero_net_momentum}
...
We begin on a perfect FCC lattice, every atom sitting precisely at its ideal 
lattice site. This is a highly artificial initial condition: the atoms have not 
yet had time to settle into their natural configurations, and the kinetic and 
potential energies are far from their equilibrium balance. If we were to start 
collecting data immediately, every measurement would be not accurate. The $g(r)$ would reflect the perfect FCC geometry rather than true 
thermal structure; the energy would still be drifting toward its equilibrium 
value.

The equilibration phase gives the system time to relax. However, simply running 
the integrator and waiting is not enough, we also need to drive the system 
toward a \textit{specific} target temperature $T^*$. This is where velocity 
rescaling comes in.

When atoms are placed on the FCC lattice and given random initial velocities, 
the instantaneous temperature (computed from the kinetic energy via 
$T^* = \sum_i v_i^2 / 3N$) will not match the target. More 
importantly, as the system relaxes from the artificial lattice, potential energy 
gets converted into kinetic energy and vice versa, the temperature fluctuates 
wildly in the early steps. Without artificial involvement, the system might wander off to 
a temperature far from what we intend to simulate.

To prevent this, every few steps during equilibration we compute the current 
instantaneous temperature $T^*_{\mathrm{inst}}$ and rescale all velocities by 
the factor

\begin{equation}
    \lambda = \sqrt{\frac{T^*_{\mathrm{target}}}{T^*_{\mathrm{inst}}}}
\end{equation}

so that $v_i \to \lambda v_i$ for every atom. This directly adds or removes 
kinetic energy from the system, maintaining the temperature to be the same as the target temperature. 
We repeat this every 10 steps for the first 1000 steps, giving the system 
repeated nudges until it settles. This is not a physical thermostat, it is a 
blunt correction tool used only to prepare the system. Once the energies have 
settled into steady oscillations and the temperature is stable around $T^*$, we 
switch off rescaling entirely and let the system evolve freely under the NVE 
ensemble. This is the production phase, and only now do we trust the data.
...
\begin{equation}
    ...
    \label{eq:mb}
\end{equation}
After sampling, the mean velocity is subtracted component-wise so that $\sum_i \mathbf{v}_i = \mathbf{0}$ exactly. In our test run, the residual momentum after this step was $|\mathbf{P}^*| = 1.77 \times 10^{-14}$, which is numerical noise at the level of floating-point precision.
...
\begin{enumerate}
    ...
    \item \textbf{Production} (steps $N_{\text{equil}}$ to $N_{\text{total}}$): Thermostat is switched off. The system evolves freely; all energies, temperatures are accumulated.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Kinetic energy $K^*$, potential energy $U^*$, and total energy $E^*$ over all 5000 timesteps for $N=256$, $\rho^* = 0.8442$, $T^* = 1.0$, $\delta t^* = 0.005$. The orange-shaded region is the equilibration phase (steps 0 to 2500), where velocity rescaling periodically adjusts $K^*$; the blue-shaded region is the production phase (steps 2500 to 5000), where all quantities evolve freely. The flatness of $E^*$ (red) in the production phase is the primary confirmation of physical correctness.}
    \label{fig:m5_energy}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Instantaneous reduced temperature $T^*_\text{inst}$ over all 5000 timesteps. During equilibration, the thermostat acts like a strict speed limiter; every 10 steps it rescales all velocities to force the temperature back to exactly $T^* = 1.0$ (red dashed). In production, the limiter is switched off and the temperature floats freely to $\langle T^* \rangle = 1.033$ (black dotted). This small offset is not an error; with only $N = 256$ atoms, there is always a minor statistical gap between the target temperature and the one the freely-running system settles at. What matters is that the temperature is stable and not drifting.}
    \label{fig:m5_temp}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        $\langle \Estar \rangle$ (production)              & N/A                   & $-717.16$ \\
        RMS fluctuation $\Delta \Estar$                    & N/A                   & $5.34 \times 10^{-2}$ \\
